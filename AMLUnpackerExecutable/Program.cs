...
namespace AMLUnpackerExecutable
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("AMLUnpacker - by Ricky Divjakovski\n");
            if (args[0] == "-u")
            {
                if (args.Length == 3)
                {
                ...
                else Console.WriteLine("Usage: AMLUnpacker.exe -u aml_upgrade_package OutputFolder\n  -u | Unpack image");
            }
            else if (args[0] == "-h") Console.WriteLine("Usage: AMLUnpacker.exe -u aml_upgrade_package OutputFolder\n  -u | Unpack image");
            else Console.WriteLine("Usage: AMLUnpacker.exe -u aml_upgrade_package OutputFolder\n  -u | Unpack image");
        }

        static void HexSplit(string inputFile, string outputFile, string startAddress, string endAddress)
        {
